...
public class DriveSubsystem extends SubsystemBase {
    ...
    public void controllerDriveFieldCentricFacingPose(Pose2d pose) {
        controllerDriveFieldCentricFacingPose(pose, Rotation2d.kZero);
    }

    public void controllerDriveFieldCentricFacingPose(Pose2d pose, Rotation2d offset) {
        ...
        controllerDriveFieldCentricFacingDir(dir.rotateBy(offset));
    }

    public void controllerDriveRobotCentricFacingHub() {
        controllerDriveRobotCentricFacingTranslation(Constants.FieldConstants.HUB_BLUE.toTranslation2d(),
                Rotation2d.k180deg);
    }

    public Command controllerDriveRobotCentricFacingHubCommand() {
        return run(this::controllerDriveRobotCentricFacingHub)
                .withName("driveControllerRobotCentricFacingHub");
    }

    public void controllerDriveRobotCentricFacingTranslation(Translation2d translation) {
        controllerDriveRobotCentricFacingTranslation(translation, Rotation2d.kZero);
    }

    public void controllerDriveRobotCentricFacingTranslation(Translation2d translation, Rotation2d offset) {
        Rotation2d dir = getPose().getRotation();

        Translation2d trans = translation
                .minus(getPose().getTranslation());
        if (trans.getNorm() > 0.1) {
            dir = trans.getAngle();
        } // Don't auto rotate when less than 10cm away from target

        controllerDriveRobotCentricFacingDir(dir.rotateBy(offset));
    }
  ...
  public void driveRobotRelative(ChassisSpeeds speeds) {
    ...
    public Command controllerDriveFieldCentricFacingPoseCommand(DoubleSupplier xSupplier, DoubleSupplier ySupplier) {
        return run(() -> controllerDriveFieldCentricFacingPose(
                new Pose2d(xSupplier.getAsDouble(), ySupplier.getAsDouble(), Rotation2d.kZero)));
    }
    ...
    public void updatePose() {
        ...
        if (limelightPoseSupplier != null && limelightTimestampSupplier != null && turretAngle != null
                && doUpdate != null
                && doUpdate.get()) {
            Pose2d robotPose;
        ...
        return new Pose2d(
                cameraPose.getTranslation().plus(
                        new Translation2d(Constants.TURRET_RADIUS.in(Units.Meters), 0)
                                .rotateBy(
                                        Rotation2d.fromDegrees(robotRot.getDegrees() + (turretSubsystem.getHeading()))))
