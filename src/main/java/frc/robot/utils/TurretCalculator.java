...
import static frc.robot.Constants.DISTANCE_ABOVE_FUNNEL;
import static frc.robot.Constants.FLYWHEEL_RADIUS;
import static frc.robot.Constants.TURRET_MIN_ANGLE;
import static frc.robot.Constants.TURRET_MAX_ANGLE;
import static frc.robot.Constants.ROBOT_TO_TURRET_TRANSFORM;
import static frc.robot.Constants.SHOT_MAP;
import static frc.robot.Constants.TOF_MAP;
...
public class TurretCalculator {
    public static Distance getDistanceToTarget(Pose2d turretPose, Translation3d target) {
        return Meters.of(turretPose.getTranslation().getDistance(target.toTranslation2d()));
    }
    ...
    public static Angle calculateAzimuthAngle(Pose2d robot, Translation3d target/* , Angle currentAngle */) {
        Pose2d turretPose = new Pose3d(robot)
        ...
        return calculateAzimuthAngle(turretPose, direction.getAngle().getMeasure()/* , currentAngle */);
    }

    // calculates the angle of a turret relative to the turret to hit a target
    public static Angle calculateAzimuthAngle(Pose2d turretPose, Angle fieldRelativeAngle/* , Angle currentAngle */) {
        double angle = MathUtil.inputModulus(
                new Rotation2d(fieldRelativeAngle).minus(turretPose.getRotation()).getRotations(), -0.5, 0.5);
        // double current = currentAngle.in(Rotations);
        // if (current > 0 && angle + 1 <= TURRET_MAX_ANGLE.in(Rotations)) angle += 1;
        // if (current < 0 && angle - 1 >= TURRET_MIN_ANGLE.in(Rotations)) angle -= 1;
    ...
    public static ShotData iterativeMovingShotFromMap(
            Pose2d robot, ChassisSpeeds fieldSpeeds, Translation3d target, int iterations) {
        Pose2d turretPose = new Pose3d(robot).transformBy(ROBOT_TO_TURRET_TRANSFORM).toPose2d();
    ...
    public record ShotData(double exitVelocity, double hoodAngle, Translation3d target) {
        ...
        public LinearVelocity getLinearExitVelocity() {
            return angularToLinearVelocity(RPM.of(this.exitVelocity), FLYWHEEL_RADIUS);
        }

        public AngularVelocity getAngularExitVelocity() {
            return RPM.of(this.exitVelocity);
        }
